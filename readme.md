...
- **Базы 1С** — обычный список баз 1С (`ibases.v8i`, тот же, что видит окно
  ...
  - запуск идёт напрямую платформой: файловой базе передаётся путь (`/F`),
    серверной — «сервер\база» (`/S`), веб-базе — адрес (`/WS`);
...
`.1С/switch-base.ps1` и `.1С/base-profiles/*.psd1`. Готовый набор таких
скриптов (`.1С/`, включая статический контроль BSL перед обновлением базы)
лежит в [`scripts/.1С/`](scripts/.1С/) этого репозитория — скопируйте его в
корень своего проекта, см. [`scripts/.1С/README.md`](scripts/.1С/README.md).
Само дерево метаданных и ER-диаграммы читают файлы с диска напрямую, без
запуска 1С.
